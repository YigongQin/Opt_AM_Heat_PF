...
# while iter_sol < 80:
 ...
 #               get_initial_contour_T(unew_interp, Gnew_interp, Rnew_interp, simu.source_x, theta ) 
    ...
    print(t)
    if kk*dt >= float(sys.argv[1])/phys.time_scale: 
        ...
        xd = len_um(xx)
        yd = len_um(yy)
        ud = temp_dim(np.reshape(u, (simu.nx, simu.ny), order='F'))
        mask = (u > 0 ) & (u < 1)
        lat_cp = inv_Ste* pi/2. * np.sin(pi * u) * mask
        lat_cp = np.reshape(lat_cp, (simu.nx, simu.ny), order='F')
    ...
    if iter_sol%10 == 0:
       ...
       # plt.suptitle('t= %1.2f ms'%(t*self.simu.dt*self.phys.time_scale*1e3))
        ...
        h1 = ax[0].pcolormesh( xd,yd, ud, vmin = 850, vmax = 950, cmap = 'plasma')
        ax[0].contour(xd,yd,ud, [phys.Ts, phys.Tl])
        ...
        lat_cp = inv_Ste* pi/2. * np.sin(pi * u) * mask
        lat_cp = np.reshape(lat_cp, (simu.nx, simu.ny), order='F')
       # h2 = ax[1].pcolormesh( xd[1:-1,1:-1],yd[1:-1,1:-1], gradT_um(G)*vel_dim(R), \
      #                       vmin  = 50, vmax = 250, cmap = 'inferno')
        h2 = ax[1].pcolormesh(xd, yd, lat_cp, cmap = 'hot')
      #  ax[1].contour(xd,yd,Rd, [0])\
        ax[1].set_xlabel('x [um]')
        ax[1].set_ylabel('y [um]')
        ax[1].set_title('L/C_p')
        #ax[1].set_title('dT_dt [K/s]')
        ax[1].set_aspect('equal')
        ax[1].set_xlim(xlim,0)
       ...
       # plt.savefig('./figs/'+sys.argv[1]+'_GR_frame'+str(t)+'.png',dpi=600, bbox_inches='tight')    
        ...
        '''
sio.savemat('time'+sys.argv[1]+'.mat',{'x':xd,'y':yd,'T':u2d,'lat':lat_cp})
